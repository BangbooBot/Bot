...
#[async_trait]
impl ModalHandler for Ban {
    fn custom_id(&self) -> String {
        "modal/moderate/ban".into()
    }
    ...
    async fn run(
        ...
        ban_action(&ctx, interaction, &users, reason).await;
...
pub async fn ban_action(
    ...
    let embed = officer_cui_action_embed(user, "Ban", &success, &failed, reason);
